...
private:
  ...
  TxAllocationContext(llvm::Value *_value,
      ...
      : value(_value), callHistory(_callHistory) {}
...
private:
  ...
  TxAllocationInfo(ref<TxAllocationContext> &_context, ref<Expr> _base,
                   uint64_t _size)
      : context(_context), base(_base), size(_size) {}
...
private:
  ...
  TxVariable(const TxVariable &src)
      : allocInfo(src.allocInfo), offset(src.offset),
  ...
  TxVariable(ref<TxAllocationInfo> _allocInfo, ref<Expr> _offset)
      : allocInfo(_allocInfo), offset(_offset) {
    isConcrete = false;
    concreteOffset = 0;
...
private:
  ...
  TxStateAddress(ref<TxVariable> _variable, ref<Expr> _address,
      ...
      : variable(_variable), address(_address),
...
private:
  ...
  TxStateValue(llvm::Value *value,
      ...
      : value(value), valueExpr(_valueExpr),
